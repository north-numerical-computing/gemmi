  ...
  for (size_t i = 0; i < otherDim; ++i) {
    for (size_t j = 0; j < innerDim; ++j) {
      ...
      for (size_t slice = 0; slice < split.numSplits; ++slice) {
        const auto digit = static_cast<fp_t>(split.memory[index + slice * matrixSize]);
...
void runSplitRoundTripTests() {
  constexpr size_t bitsPerSlice = 7;
  ...
  for (auto strategy : {splittingStrategy::bitMasking,
    ...
    DYNAMIC_SECTION("type="
      ...
      MatrixSplit<int8_t, fp_t> split(matrix, m, n, strategy, numSplits,
                                      bitsPerSlice, normalisationDimension::byRows);
    ...
    DYNAMIC_SECTION("type="
                    ...
                    << ", dimension=byCols") {
      MatrixSplit<int8_t, fp_t> split(matrix, m, n, strategy, numSplits,
                                      bitsPerSlice, normalisationDimension::byCols);
      const auto reconstructed = reconstructFromSplit(split);
      requireBitwiseIdenticalVectors(reconstructed, matrix);
    }
  }
 ...
 *******************************/
template <typename fp_t>
std::vector<fp_t> makeRandomMatrix(size_t rows, size_t cols, std::uint64_t seed) {
        ...
        for (size_t numSplitA : {10u, 15u}) {
          for (size_t numSplitB : {10u, 15u}) {
            DYNAMIC_SECTION("type="
                            ...
                            << ", numSplitA=" << numSplitA
                            << ", numSplitB=" << numSplitB) {
              const auto A = makeRandomMatrix<fp_t>(m, k, 0xA11CEull + 31 * numSplitA + 131 * numSplitB);
              const auto B = makeRandomMatrix<fp_t>(k, n, 0xB0Bull + 17 * numSplitA + 97 * numSplitB);
